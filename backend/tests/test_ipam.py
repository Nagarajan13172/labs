...
async def test_allocations_are_sequential_and_skip_reserved(client: AsyncClient) -> None:
    ...
    first = await ipam_service.allocate_lab_ip()
    second = await ipam_service.allocate_lab_ip()
...
async def test_concurrent_allocations_are_unique(client: AsyncClient) -> None:
    ips = await asyncio.gather(*(ipam_service.allocate_lab_ip() for _ in range(25)))
    assert len(set(ips)) == 25  # no collisions under concurrency


async def test_released_ip_is_reused(client: AsyncClient) -> None:
    a = await ipam_service.allocate_lab_ip()
    b = await ipam_service.allocate_lab_ip()
    await ipam_service.release_lab_ip(a)
    reused = await ipam_service.allocate_lab_ip()
    assert reused == a  # released address comes back before a fresh one
    assert b != reused


async def test_lab_and_vpn_pools_are_independent(client: AsyncClient) -> None:
    lab = await ipam_service.allocate_lab_ip()
    vpn = await ipam_service.allocate_vpn_ip()
    assert lab == "172.30.0.2"
    assert vpn == "10.8.0.2"  # separate counter + subnet
